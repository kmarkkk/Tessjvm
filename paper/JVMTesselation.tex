...
\end{figure}
Same as the DaCapo setup in ~\ref{expsetup}, we run both Cassandra and YCSB as OS$^{v}$ guest domains upon Xen hypervisor on the exactly same machine and infrusturcture. Basically, the architecture of our experiment is set up as shown in Figure~\ref{cassandrasetup}. We pair each YCSB domain with exactly one Cassandra domain, so each Cassandra instance will receive the full number of requests configured in the YCSB workload file. The main reason for this independent setup is to maintain consistent workload across multiple Cassandra instances. It's possible to start one YCSB to issue requests to all Cassandra domains, but the automatic load balance between nodes happened in YCSB could be unpredicable and may skew the result. All domains are configured in a private LAN through a Xen bridge, to minimize the network influence on request latency and throughput. Given the high memory demand from Cassandra DBMS and the 12GB memory cap of the test machine, we limit each Cassandra domain to 2GB memory and each YCSB domian to 512MB memory. This means we can scale up to at most 4 Cassandra JVMs. All domains run with 6 virtual cpus on the default Xen credit scheduler. Since the Cassandra domain has only 2GB in total, we cap the record count in each database to 5000, with 10 fields each. The first field is the primary key, and the 9 left are varchar fields. We set the request count to 100,000, and run two kind of core workloads. The first one is update heavy, which consists of 50\% read and 50\% update. Typical application for the workload is session store which records recent actions. The second workload is read-modify-write heavy. Related application can be user database, where user records are read and modified by the user or to record user activity. This one includes 50\% read and 50\% read-modify-write. Both workloads use zipfian distribution for requests. %TODO expaln this
The data size is set to 1K, so each field gets 100 bytes. For each experiement, we first run a load phrase to populate the database, and then run one iteration of the workload run phrase to help Cassandra get warm up. After that, we run 5 iterations of run phrase and record the overall as well as operation level lantency and throughput. The final value is the average of those numbers across multiple JVMs during 5 iterations. Besides that, we record the GC stats.

\subsubsection{Overall Performance}
We run the workload with 1,2,4 Cassandra instances.
