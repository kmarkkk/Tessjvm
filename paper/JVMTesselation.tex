...
We run the Cassandra domains with flags \textit{-XX:+UseParNewGC} and \textit{-XX:+UseConcMarkSweepGC} to turn on the \textit{CMS} collector with the same experiment setup above. We record the GC time for both minor and major(mark and sweep) collection phrases that occur during the execution of the 5 iterations of run phrase after warm up.
